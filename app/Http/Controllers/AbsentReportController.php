...
class AbsentReportController extends Controller
{
    public function index(Request $request)
    {
        $validated = $request->validate([
            ...
            'employee_category' => 'nullable|string', // 🔥 1. Add this
        ]);
        ...
        $employeeCategory = $validated['employee_category'] ?? null; // 🔥 2. Extract this
        $currentPage = max((int)$request->query('page', 1), 1);

        if ($mode === 'daily' && empty($validated['date'])) {
            return response()->json(['message' => 'date is required for daily mode'], 422);
        }

        if ($mode === 'monthly' && (empty($validated['month']) || empty($validated['year']))) {
            return response()->json(['message' => 'month and year are required for monthly mode'], 422);
        }

        // 🔥 3. Pass Category to the methods
        if ($mode === 'daily') {
            $rows = $this->getDailyRows($validated['date'], $session, $search, $companyId, $departmentId, $employeeCategory);
        } else {
            $rows = $this->getMonthlyRows((int)$validated['month'], (int)$validated['year'], $session, $search, $companyId, $departmentId, $employeeCategory);
        }
    ...
    private function getMonthlyRows(int $month, int $year, string $session, ?string $search, $companyId, $departmentId, $employeeCategory = null) 
    {
        $today = Carbon::today();
        $monthStart = Carbon::createFromDate($year, $month, 1)->startOfMonth();
        $monthEnd = Carbon::createFromDate($year, $month, 1)->endOfMonth();

        if ($monthStart->gt($today)) return collect();
        ...
        foreach ($period as $day) {
            ...
            // 🔥 4. Pass Category down
            $dailyRows = $this->getDailyRows($date, $session, $search, $companyId, $departmentId, $employeeCategory);
        ...
        return $rows->sortBy([['date', 'desc'], ['name', 'asc']])->values();
    }
    ...
    private function getDailyRows(string $date, string $session, ?string $search, $companyId, $departmentId, $employeeCategory = null) 
    {
        ...
        if ($companyId) $rosteredQuery->where('company_id', $companyId);
        if ($departmentId) $rosteredQuery->where('department_id', $departmentId);

        $rosteredRows = $rosteredQuery->get(['employee_id', 'company_id', 'department_id']);
        $rosteredEmployeeIds = $rosteredRows->pluck('employee_id')->unique()->values();

        if ($rosteredEmployeeIds->isEmpty()) return collect();
            ...
            ->where(function ($q) use ($date) {
                $q->whereDate('leave_date', $date)
                    ->orWhere(function ($q2) use ($date) {
                        $q2->whereDate('leave_from', '<=', $date)->whereDate('leave_to', '>=', $date);
                    });
            })
            ->select('employee_id', 'status', 'is_half_day', 'period', 'leave_date', 'leave_from', 'leave_to', 'leave_duration', 'leave_type')
            ->get();

        $halfMorningLeaveIds = $approvedLeaves->filter(function ($lv) {
            return ($lv->is_half_day || floatval($lv->leave_duration) == 0.5) && strtolower((string)$lv->period) === 'morning';
        })->pluck('employee_id')->unique();

        $halfAfternoonLeaveIds = $approvedLeaves->filter(function ($lv) {
            return ($lv->is_half_day || floatval($lv->leave_duration) == 0.5) && strtolower((string)$lv->period) === 'afternoon';
        })->pluck('employee_id')->unique();

        $absentEmployeeIds = collect($rosteredEmployeeIds)->diff($attendedEmployeeIds)->unique();

        if ($session === 'morning') {
            $absentEmployeeIds = $absentEmployeeIds->diff($halfMorningLeaveIds)->unique();
        } elseif ($session === 'afternoon') {
            $absentEmployeeIds = $absentEmployeeIds->diff($halfAfternoonLeaveIds)->unique();
        }
        ...
        $absentEmployeeIds = $absentEmployeeIds->diff($holidayEmployeeIds->unique())->unique()->values();

        if ($absentEmployeeIds->isEmpty()) return collect();
        ...
        // 🔥 5. ADD CATEGORY FILTER HERE 🔥
        if ($employeeCategory) {
            $employeeQuery->whereHas('compensation', function ($q) use ($employeeCategory) {
                $q->where('employee_category', $employeeCategory);
            });
        }
        ...
        $rows = $employees->map(function ($employee) use ($date, $approvedLeaves, $approvedNoPayRecords) {
            ...
            return [
                ...
                'leave_or_nopay' => $leave ? 'Approved Leave' : ($approvedNoPay ? 'NoPay' : 'Absent'),
            ];
        });
