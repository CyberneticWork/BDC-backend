...
class SalaryProcessController extends Controller
{
    ...
    // ✅ MAIN: now returns bonuses array too
    public function getEmployeesByMonthAndCompany(Request $request)
    {
        $month = $request->query('month');
        $year = $request->query('year');
        ...
        // ✅ IMPORTANT:
        // - Assumes tables: bonuses, employee_bonuses
        // - bonuses columns: id, bonus_name, bonus_code, amount, company_id, department_id, status
        // - employee_bonuses columns: id, employee_id, bonus_id, custom_amount, is_active
        //
        // If your actual column/table names differ, update only in this SQL bonuses subquery.
        $query = "
            SELECT
                e.id,
                e.attendance_employee_no AS emp_no,
        ...
        foreach ($results as $result) {
            ...
            // Gross = adjusted basic + allowances + bonuses + OT + KPI bonus
            $grossSalary = $adjustedBasic + $totalAllowances + $totalBonuses + $morning_ot_fees + $night_ot_fees + $kpiBonusAllowance;
            ...
            $employeeData['salary_breakdown'] = [
                ...
                'total_bonuses' => round($totalBonuses, 2), // ✅ helpful for frontend too
        ...
        try {
            ...
            foreach ($request->data as $employeeData) {
                ...
                salary_process::create([
                    ...
                    'allowances' => $employeeData['allowances'] ?? null,
                    'deductions' => $employeeData['deductions'] ?? null,
                    'bonuses' => $employeeData['bonuses'] ?? null, // ✅ save bonuses
                    'salary_breakdown' => $employeeData['salary_breakdown'] ?? null,
                    'month' => $month,
                    'year' => $year,
                ]);
    ...
    public function getProcessedSalaries()
    {
        ...
        $response = $processedSalaries->map(function ($salary) {
            return [
                ...
                'allowances' => $salary->allowances,
                'deductions' => $salary->deductions,
                'bonuses' => $salary->bonuses, // ✅ include in processed response
            ];
        });
    ...
    // ✅ UPDATED: now returns bonus too for template download
    public function fetchExcelData(Request $request)
    {
        $employees = employee::whereIn('id', $request->selectedEmployees)
            ->get([
        ...
        if ($request->bulkActionType == 'allowance') {
            ...
            // ⚠️ assumes table/columns: bonuses.id, bonuses.bonus_name
            $bonus = DB::table('bonuses')
                ->where('id', $request->bulkActionId)
                ->get(['id', 'bonus_name']);
        }
