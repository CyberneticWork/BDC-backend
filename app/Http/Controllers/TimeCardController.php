...
class TimeCardController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        /*
         ========================================================================
         මෙම කොටස Mid-Shift breaks සඳහා අයින් කර ඇත (Commented OUT).
         දැන් එකම දවසේ IN/OUT කිහිපයක් වදින්න පුළුවන්.
         ========================================================================

        $isMarkingIn = in_array($status, ['IN', 'Late Coming']);
        $isMarkingOut = in_array($status, ['OUT', 'Early OUT']);

        if ($isMarkingIn) {
            $alreadyIn = time_card::where('employee_id', $employee->id)
                ->where('date', $validated['date'])
                ->whereIn('status', ['IN', 'Late Coming'])
                ->whereNull('deleted_at')
                ->exists();
            ...
            if ($alreadyIn) {
                return response()->json(['message' => 'This employee is already marked as IN for this date.'], 409);
            }
        } elseif ($isMarkingOut) {
            ...
            if ($alreadyOut) {
                return response()->json(['message' => 'This employee is already marked as OUT for this date.'], 409);
            }
        }
        */
    ...
    public function attendance(Request $request)
    {
        ...
        if ($entryType === 1) {
            ...
            if ($alreadyIn) {
                return response()->json(['message' => 'Duplicate attendance record. Already marked IN.'], 409);
            }
        } elseif (in_array($entryType, [0, 2])) {
            $alreadyOut = time_card::where('employee_id', $employee->id)
                ->where('date', $request->date)
                ->whereIn('entry', [0, 2])
                ->whereNull('deleted_at')
            ...
            if ($alreadyOut) {
                return response()->json(['message' => 'Duplicate attendance record. Already marked OUT.'], 409);
            }
        }
        */
 ...
 //===============================================
 // ================================================================
    // Mid-Shift Breaks (අතරමැදි ගමන්) ලබා ගැනීමේ API එක
    // ================================================================
    public function getAllIntermediateMovements(Request $request)
    {
        ...
        // එදා දවසට අදාළ ඔක්කොම Punches ටික ගන්නවා
        ...
        foreach ($allPunches as $empId => $punches) {
            // රෙකෝඩ්ස් 2ක් හෝ ඊට අඩු නම් මැද ගමන් නෑ (ඒවා First & Last වෙනවා)
            if ($punches->count() <= 2) continue; 

            // පළවෙනි එක සහ අන්තිම එක අයින් කරලා මැද ටික ගන්නවා
            ...
            for ($i = 0; $i < $middlePunches->count(); $i++) {
                ...
                // එකක් OUT, ඊළඟ එක IN විදිහට පිළිවෙලට තියෙනවා නම් (Perfect Pair)
                if (in_array(strtoupper($current->status), ['OUT', 'EARLY OUT']) && 
                    ...
                    $i++; // Pair එකක් නිසා ඊළඟ එක skip කරනවා

                } else {
                    // පිළිවෙලට නැත්නම් (e.g. IN විතරක් හෝ OUT විතරක් මැද තිබ්බොත්)
...
class TimeCardController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        // අලුතින් යෙදූ OT Update ලොජික් එක (IN සහ OUT දෙකටම වැඩ කරයි)
        if (in_array($finalStatus, ['OUT', 'EARLY OUT'])) {
            if ($pairedInCard) {
                $referenceDate = $actual_date ?? $pairedInCard->date;
                $this->processOvertimeForOutPunch($employee, $timeCard, $pairedInCard, $shift, $referenceDate);
            ...
            // Edit කළේ IN Card එක නම්, ඊට අදාළ OUT Card එක හොයාගෙන OT එක අලුතින් ගණනය කරනවා
            $pairedOutCard = time_card::where('employee_id', $employee->id)
                ->whereIn('status', ['OUT', 'Early OUT'])
                ->where(function ($q) use ($timeCard) {
                    $q->where('actual_date', $timeCard->date)
    ...
    // --- අලුත් කරපු Overtime Process Function එක (Date එකත් එක්කම Save වෙනවා) ---
    private function processOvertimeForOutPunch(
        employee $employee,
        time_card $outCard,
        time_card $inCard,
        ...
        if ($hours['total'] <= 0) {
            // OT නැත්නම් තියෙන Record එක මකලා දානවා
            over_time::where('time_cards_id', $outCard->id)->delete();
            return;
        }
    ...
    // අලුත් UpdateOrCreate Function එක (Old records මකන්නේ නැතුව Update කරයි)
    private function saveOrUpdateOtRecord($timeCardId, $data) {
        $ot = over_time::withTrashed()->where('time_cards_id', $timeCardId)->first();
        if ($ot) {
            $ot->restore(); // Delete වෙලා තිබුණොත් Restore කරනවා
            $ot->update($data);
        } else {
            $data['time_cards_id'] = $timeCardId;
            over_time::create($data);
