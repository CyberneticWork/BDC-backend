    ...
    public function store(Request $request): JsonResponse
    {
        $validator = Validator::make(
            ...
            [
                ...
                'casual_leave_days' => ['nullable', 'integer', 'min:0'], // අලුතින් එකතු කළා
        ...
        // Business validation (Sri Lankan Law Logic)
        if ($employeeType === 'probation') {
            // Probation (First Year) employees only get Casual Leaves (1 per 2 months = max 7 usually)
            // They DO NOT get Annual Leaves in the first year.
            if (!array_key_exists('casual_leave_days', $validated) || $validated['casual_leave_days'] === null) {
                return response()->json([
                    'message' => 'Casual leave days (max 7) is required for probation/first-year employees.'
                ], 422);
            }

            // probation employees should not carry quarter data
            $validated['number_of_quarters'] = null;
            $validated['annual_leave_days'] = 0; // No annual leaves for first year
            $validated['quarters'] = [];
        }
        ...
        if ($employeeType === 'permanent') {
            ...
            $validated['casual_leave_days'] = null;
        }
        ...
        $setting = DB::transaction(function () use ($settingData, $normalizedQuarters) {
            $setting = LeaveSetting::create([
                ...
                'casual_leave_days' => $settingData['casual_leave_days'] ?? null, // Added
    ...
    public function update(Request $request, LeaveSetting $leaveSetting): JsonResponse
    {
        $validator = Validator::make(
            ...
            [
                ...
                'annual_leave_days' => ['nullable', 'integer', 'min:0'],
                'casual_leave_days' => ['nullable', 'integer', 'min:0'], // Added
                'number_of_quarters' => ['nullable', 'integer', 'min:1', 'max:12'],
                'quarters' => ['nullable', 'array'],
        ...
        if ($targetType === 'probation') {
            $casualLeaveDays = $validated['casual_leave_days'] ?? $leaveSetting->casual_leave_days;

            if ($casualLeaveDays === null) {
                return response()->json([
                    'message' => 'Casual leave days is required for probation/first-year employees.'
                ], 422);
            }

            $validated['casual_leave_days'] = $casualLeaveDays;
            $validated['annual_leave_days'] = 0; // Ensure 0
            $validated['number_of_quarters'] = null;
        ...
        $updated = DB::transaction(function () use ($leaveSetting, $settingData, $normalizedQuarters, $targetType) {
            $leaveSetting->update([
                ...
                'annual_leave_days' => $settingData['annual_leave_days'] ?? $leaveSetting->annual_leave_days,
                'casual_leave_days' => $settingData['casual_leave_days'] ?? $leaveSetting->casual_leave_days,
                'number_of_quarters' => $settingData['number_of_quarters'] ?? null,
            ...
            ->map(function ($setting) {
                return [
                    ...
                    'total_leave_days' => $setting->employee_type === 'probation'
                        ? ($setting->casual_leave_days ?? 0) // Updated to casual leaves
